...
public interface ClientDao {
	...
	//	查找成交用户
//	@ResultMap("getClient")
	@Select("select * from user where company_id = #{companyId} and open_id in (SELECT open_id from user_id where user_identity in (1,3))")
	public List<Client> limitClientsAccomplish (Integer companyId);
	
	//	未成交用户
	@Select("select * from user where company_id = #{companyId} and open_id in (SELECT open_id from user_id where user_identity = 4) limit #{page},#{limit}")
	public List<Client> limitClientsFail(Integer companyId,Integer page,Integer limit);
	
	@Select("select count(*) from user where company_id = #{companyId} and open_id in (SELECT open_id from user_id where user_identity = 4)")
	public Integer getClientCountFail(Integer companyId);
	
	//	全机构用户
//	@ResultMap("getClient")
	@Select("select * from user where company_id = #{companyId} and open_id in (SELECT open_id from user_id where user_identity in (1,3,4)) limit #{page},#{limit}")
	public List<Client> limitClients(Integer companyId,Integer page,Integer limit);
	
	@Select("select count(*) from user where company_id = #{companyId} and open_id in (SELECT open_id from user_id where user_identity in (1,3,4))")
