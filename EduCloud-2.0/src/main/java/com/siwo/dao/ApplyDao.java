...
public interface ApplyDao {
	...
	//	添加
	@Insert("insert into apply (apply_title,apply_slideshow,apply_img,apply_show,apply_creator,apply_creation_time,school_id,apply_start_time,apply_stop_time,apply_address,apply_principal,apply_phone,apply_telephone) "
			+ "values (#{applyTitle},#{applySlideshow},#{applyImg},#{applyShow},#{applyCreator},#{applyCreationTime},#{schoolId},#{applyStartTime},#{applyStopTime},#{applyAddress},#{applyPrincipal},#{applyPhone},#{applyTelephone}) ")
	...
	//	修改是否展示
	@Update("UPDATE `apply` SET `apply_show` = #{applyShow} where school_id = #{schoolId}")
	public Integer updateApplyShow(Integer applyShow,Integer schoolId);
}
