	...
	public Map<String, Object> addXqLearningOutcomes(List<XqLearningOutcomes> outcomes) {
		...
		try {
			for (XqLearningOutcomes outcome : outcomes) {
				XqLearningOutcomes xqLearningOutcomes = dao.queryXqLearningOutcomesByStudentId(outcome.getStudentId(),
						outcome.getClassDayId(), outcome.getXqTypeId(), outcome.getClassId());
			...
			for (XqClassDay xqClassDay : xqClassDays) {
				...
				for(Student student : students) {
					...
					if (xqLearningOutcomes == null || xqLearningOutcomes.size() == 0) {
						...
						List<XqType> xqTypes = allDao.queryXqTypes(schoolId);
						for (XqType xqType : xqTypes) {
							XqLearningOutcomes xqLearningOutcome = new XqLearningOutcomes(null, xqClassDay.getClassId(), student.getStudentId(), xqType.getXqTypeId(), xqClassDay.getClassDayId(), 3);
