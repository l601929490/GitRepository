...
public class IseDemo extends WebSocketListener {
	...
	private static final String appid = "5f891b27";// 控制台获取
	private static final String apiSecret = "1d60e604100bab80c919beee61425256";// 控制台获取
	private static final String apiKey = "c601ea45bba054c55d9a57554ddd1383";// 控制台获取

	private static final String sub = "ise";// 服务类型sub,开放评测值为ise
	
	private static String ent;// 语言标记参数 ent(cn_vip中文,en_vip英文)

	// 题型、文本、音频要请注意做同步变更(如果是英文评测,请注意变更ent参数的值)
	private static String category = "read_sentence";// 题型
	...
	/*
	 * private int aus = 1; private static Date dateBegin = new Date();// 开始时间
	 * private static Date dateEnd = new Date();// 结束时间 private static final
	 * SimpleDateFormat sdf = new SimpleDateFormat("yyy-MM-dd HH:mm:ss.SSS");
	 */
	private static long beginTime = (new Date()).getTime();
	private static long endTime = (new Date()).getTime();

	public synchronized Map<String, Object> star(String ent,String text,String file) {
		...
		try {
			...
			String url = authUrl.replace("http://", "ws://").replace("https://", "wss://");// 将url中的 schema
			
			Request request = new Request.Builder().url(url).build();
			client.newWebSocket(request, new IseDemo());
			
			while (StringUtils.isEmpty(IseDemo.total_score)) {
				System.out.println("评测中");
				Thread.sleep(300);
			}
			//	总分
			map_2.put("totalScore", IseDemo.map.get("total_score"));
			//	是否乱读
			map_2.put("is_rejected", IseDemo.map.get("is_rejected"));
			//	准确度
			map_2.put("accuracy_score", IseDemo.map.get("accuracy_score"));
			//	流畅度
			map_2.put("fluency_score", IseDemo.map.get("fluency_score"));
			//	完整度
			map_2.put("integrity_score", IseDemo.map.get("integrity_score"));
		} catch (Exception e) {
			// TODO: handle exception
		}

		System.out.println("最终总得分："+IseDemo.total_score);
		IseDemo.map = null;
		IseDemo.total_score = null;
		return map_2;
	}
	...
	public void onOpen(WebSocket webSocket, Response response) {
		...
		new Thread(() -> {
			// 连接成功，开始发送数据
			int frameSize = 1280; // 每一帧音频的大小,建议每 40ms 发送
									// 1280B，大小可调整，但是不要超过19200B，即base64压缩后能超过26000B，否则会报错10163数据过长错误
			int intervel = 40;
			int status = 0; // 音频的状态
...
//				System.out.println("all data is send");
	...
	private void ssb(WebSocket webSocket) {
		...
		p.add("common", new ParamBuilder().add("app_id", appid))
				.add("business", new ParamBuilder().add("category", category).add("rstcd", "utf8")
						...
						.add("sub", sub).add("ent", ent).add("tte", "utf-8").add("cmd", "ssb")
						.add("auf", "audio/L16;rate=16000").add("aue", "raw")
	...
	public void send(WebSocket webSocket, int aus, int status, String data) {
		ParamBuilder p = new ParamBuilder();
		p.add("business", new ParamBuilder().add("cmd", "auw").add("aus", aus).add("aue", "raw")).add("data",
				new ParamBuilder().add("status", status).add("data", data).add("data_type", 1).add("encoding", "raw"));
	...
	public void onMessage(WebSocket webSocket, String text) {
		...
		if (resp != null) {
			if (resp.getCode() != 0) {
				...
				IseDemo.total_score = "错误码："+resp.getCode()+"  错误原因："+resp.getMessage();
				return;
			}
...
//								+ new String(decoder.decode(resp.getData().getData()), "UTF-8"));
						xmlJiexi(new String(decoder.decode(resp.getData().getData()), "UTF-8"));
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public void xmlJiexi(String xml) {
		Document doc = null;
		try {
...
//		System.out.println("当前节点的内容："+node.getTextTrim());//当前节点名称
		...
		IseDemo.map = new HashMap<String, Object>();
		for(Attribute attr:listAttr){//遍历当前节点的所有属性
			String name=attr.getName();//属性名称
			String value=attr.getValue();//属性的值
//			System.out.println("属性名称："+name+"属性值："+value);
			if (name.equals("is_rejected")) {
				IseDemo.map.put("is_rejected", value);
			}
			if (name.equals("accuracy_score")) {
				IseDemo.map.put("accuracy_score", value);
			}
			if (name.equals("fluency_score")) {
				IseDemo.map.put("fluency_score", value);
			}
			if (name.equals("integrity_score")) {
				IseDemo.map.put("integrity_score", value);
			}
			if (name.equals("total_score")) {
				IseDemo.total_score = value;
				IseDemo.map.put("total_score", value);
				return;
			}
		}
	...
	private static class IseNewResponseData {
		...
		public String toString() {
			return "IseNewResponseData [code=" + code + ", message=" + message + ", sid=" + sid + ", data=" + data
					+ "]";
		}
