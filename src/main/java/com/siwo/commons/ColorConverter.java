...
public class ColorConverter {
	...
	public static HSL RGB2HSL(RGB rgb) {
		...
		if (del_Max == 0) {
			...
			float del_R = ((360 * (var_Max - red) / 6) + (360 * del_Max / 2)) / del_Max;
			float del_G = ((360 * (var_Max - green) / 6) + (360 * del_Max / 2)) / del_Max;
			float del_B = ((360 * (var_Max - blue) / 6) + (360 * del_Max / 2)) / del_Max;
			if (red == var_Max) {
				H = del_B - del_G;
